...
void os_semaphore_init(uint32_t *semaphore, uint32_t value) {
	*semaphore = value;
}

void os_semaphore_set(uint32_t *semaphore) {
	__disable_irq();  // keeps writes atomic
	*semaphore += 1;   // increment value
	__enable_irq();
}

void os_semaphore_wait(uint32_t *semaphore) {
	__disable_irq();  // keeps writes atomic
	while(*semaphore <=0) {
		__disable_irq();  // why  ?
		__enable_irq();
